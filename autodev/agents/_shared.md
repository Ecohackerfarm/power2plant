...
```sh
ssh -i /home/agent/.ssh/power2plant_dev -p 2222 -o StrictHostKeyChecking=no node@power2plant-app-1 "cd <WORKTREE_PATH> && <command>"
```

Always pass `DATABASE_URL` explicitly — never rely on `.env`:
```sh
ssh ... node@power2plant-app-1 "cd <WORKTREE_PATH> && DATABASE_URL=<DATABASE_URL> <command>"
```
